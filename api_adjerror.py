...
class CurrentValues(BaseModel):
    WATER: Optional[Values] = None
    FINGER: Optional[Values] = None
    AIR: Optional[Values] = None
...
def find_different(current_values: Dict[str, Dict[str, float]], standard_values: Dict[str, Dict[str, float]]):
    ...
    for key, current in current_values.items():
        if current is not None:  # Check if the category exists in the input
...
async def calculate_differences(current_values: CurrentValues):
    ...
    current_values_dict = {
        key: value for key, value in current_values.dict().items() if value is not None
    }

    # Calculate and return differences
    differences = find_different(current_values_dict, standard_values)

    # Calculate the average of each feature (only for provided categories)
    averages = {}
    for feature in next(iter(differences.values())).keys():
